...
In a project thread, the right-panel new-tab launcher has a **Trails** tab scoped to that repository and its known checkouts/worktrees. It groups sessions by workday, with the repository’s day summary above its sessions and a work-date filter. Scope and host are resolved on the server from the thread, not from persisted tab parameters. Day summaries are matched by work date and project path, so the tab includes summaries for the scoped repository. Projectless threads show an explanatory message without fetching activity.
...
Dates use Trails' configured timezone and workday boundary. Attention minutes merge overlapping user-activity intervals with the configured halo; they are an estimate, not billed time. Date-filtered session details still describe the whole session. Day summaries describe one project on one work date and appear on the matching project row. Days list only dates with observed activity, not every calendar date.

Use `nextOffset` with `--offset` to page results. Limits are 1–20, default 7; days show up to 30 projects, projects and repository workdays show their latest ten sessions, and text fields are capped at 2,048 characters. For older sessions narrow by date. If the result is too large, lower the limit. Health failures are reported separately when one endpoint still works; missing summary status is not the same as summaries being off.
...
Authenticated hubs require a separate private `~/.config/trails/reader.json` on the selected host account, issued by the hub owner with `trails auth read --server HUB_URL --output reader.json`. Its URL must match the resolved connection. Do not read or display the token in agent output. An HTTP 401 means the read credential is missing, invalid, or revoked; collector credentials cannot provide read access. Ask the owner to provision the reader file privately when needed.
