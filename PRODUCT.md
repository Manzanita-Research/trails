...
- Days are human-shaped: the working day starts around 6 am; 1 am work belongs to the evening it grew out of. The owner can choose the IANA timezone and a 4–7 am boundary on the hub.
...
- Summarization: changed sessions settle for five minutes, then durable jobs send bounded digests through one explicitly selected coding harness already installed on the hub. Sessions become one-line contribution summaries; project/day rollups are guarded against stale responses. Summaries remain off until the owner activates a harness.
...
- **Privacy boundary:** normalized metadata and bounded digests may leave a source machine for the owner's hub. Full transcript bodies and local transcript paths do not. Browser bootstrap omits source-local session IDs and digests. Only bounded digest input and Trails-owned prompts reach the selected harness and its already-configured provider; Trails never reads or stores harness credentials. Feedback leaves the browser only when sent; optional safe context contains only explicitly documented operational fields.
- **Distribution shape:** two standalone macOS executables (`arm64`, `x64`) embed Bun, SQLite, the harness adapters, and the built client. Target Macs require no runtime or repository checkout; a selected coding harness is required only for optional summaries. The product stages its own installer and artifacts; shared Manzanita release infrastructure stores immutable versioned objects in R2, publishes a short-lived alpha channel pointer, and serves the stable checksum-pinned installer at `https://releases.manzanita.dev/trails/install.sh`. Users retain ownership of the canonical database and daily backups on the hub.
- Stack: Bun, `bun:sqlite`, Vite, React 19, TypeScript, Effect, optional Tailscale Serve, launchd, local coding-harness invocation, and a separate public-write/no-public-read feedback Worker with expiring D1 records.
