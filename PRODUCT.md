...
- Summarization: changed sessions settle for five minutes, then durable jobs send bounded digests directly from the hub to one explicitly connected provider. Sessions become one-line contribution summaries; project/day rollups are guarded against stale model responses. Summaries remain off until the owner separately logs in and activates a provider.
...
- **Privacy boundary:** normalized metadata and bounded digests may leave a source machine for the owner's hub. Full transcript bodies and local transcript paths do not. Browser bootstrap omits source-local session IDs and digests. Only bounded digest input and Trails-owned prompts leave the hub when the owner activates a provider; provider credentials remain in owner-only hub files and never enter SQLite or collector traffic. Feedback leaves the browser only when sent; optional safe context contains only app version, view, revision, selected Days/Project work date, source counts, viewport dimensions, and sync-error state.
- **Distribution shape:** two standalone macOS executables (`arm64`, `x64`) embed Bun, SQLite, the provider connectors, and the built client. Target Macs require no runtime or repository checkout. The product stages its own installer and artifacts; shared Manzanita release infrastructure stores immutable versioned objects in R2, publishes a short-lived alpha channel pointer, and serves the stable checksum-pinned installer at `https://releases.manzanita.dev/trails/install.sh`. Users retain ownership of their hub, database, backups, optional tailnet, and provider credentials.
- Stack: Bun, `bun:sqlite`, Vite, React 19, TypeScript, Effect, optional Tailscale Serve, launchd, native OpenRouter/OpenAI connectors, and a separate public-write/no-public-read feedback Worker with expiring D1 records.
