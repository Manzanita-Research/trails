...
Projects are discovered from session working directories and repository context. You can rename projects and assign related projects to a shared engagement. Those edits are canonical hub state, so every browser sees the same organization.
