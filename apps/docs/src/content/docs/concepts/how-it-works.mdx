...
When work is spread across several Macs, Tailscale can privately connect them. The hub still collects its own data and remains canonical. Each additional spoke parses its own coding sessions locally and sends normalized observations to the hub every minute. When capture is enabled, a collecting Mac also sends the bounded capture record and decoded image evidence described in the [privacy boundary](/concepts/privacy).
...
- bounded capture metadata, exact minute attention, and offline image bytes;
...
## Cloudflare’s bounded roles

Cloudflare does not host the trails application or its canonical database.

When optional summary generation is enabled, an authenticated inference Worker relays bounded inputs to Workers AI. Complete transcripts are not sent through that relay.

When a tester explicitly sends beta feedback, the browser writes that report to a separate public-write Worker and expiring D1 store with no public read route. Safe context is opt-in and bounded; feedback expires after 90 days and is deleted by the next daily cleanup.

The inference relay and feedback store are separate. SQLite on the hub Mac remains canonical for trails session and organization state.
