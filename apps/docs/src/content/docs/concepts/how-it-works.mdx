...
When the owner activates summary generation, the hub invokes one installed coding harness with bounded digest input and trails-owned prompts. The harness uses its existing provider, model, login, and billing. Trails never reads, copies, refreshes, or stores the harness's credentials.
...
SQLite on the hub Mac remains canonical for trails session and organization state. Trails never retries a failed summary job through a different harness automatically.
