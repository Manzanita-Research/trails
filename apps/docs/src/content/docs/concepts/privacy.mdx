...
## May reach one selected provider

Generated summaries are optional and off by default. After the hub owner connects and separately activates a provider, the hub sends that company only trails-owned system prompts and the bounded summary input for a queued job. Complete transcripts, source files, database contents, collector traffic, and unrelated environment values are never provider input.

Current limits are:

- up to 9,000 characters for a session summary input;
- up to 12,000 characters for a day summary input;
- up to 4,000 characters of stored provider output.

Provider credentials remain on the hub in an owner-only `~/.config/trails/auth.json` file. The active provider and model live separately in `~/.config/trails/server.json`. Credentials never enter SQLite, collector traffic, browser responses, feedback, or logs. The browser receives only provider, model, login/active state, timestamps, and a closed error class.

If no provider is active or a provider is unavailable, timeline collection continues and durable summary jobs remain queued. Trails uses the session's first prompt as fallback copy and never sends the job to another provider automatically.
...
This expiring feedback store is separate from provider inference. It never becomes canonical trails state; the hub Mac’s SQLite database remains canonical for sessions and organization.

<Aside type="note" title="You hold the system">
  Your hub Mac owns the database, backups, provider selection, and provider credentials. A tailnet and summary provider are optional. Trails does not need a hosted application or inference server.
</Aside>
