...
This fallback does not mean collection failed. Timeline and project activity are stored before optional summary generation. Open **settings → summarization** to check whether a provider is active and whether its sanitized status says authentication, quota, provider rejection, timeout, protocol, or network needs attention. Summary jobs remain durable; Trails does not send them to a second provider automatically.
