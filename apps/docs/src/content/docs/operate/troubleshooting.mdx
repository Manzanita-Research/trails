...
This fallback does not mean collection failed. Timeline and project activity are stored before optional summary generation. Open **settings → summarization** to check whether a harness is active and whether its sanitized status says login, quota, harness, timeout, or protocol attention is needed. Summary jobs remain durable; Trails does not resend a failed job through a different harness automatically.
